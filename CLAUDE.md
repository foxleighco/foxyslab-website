...
- Docker passes `YOUTUBE_API_KEY`, `FLAGS_SECRET` and `FOURTHWALL_STOREFRONT_TOKEN` as **BuildKit secrets** (`secrets:` in `docker-compose.yml` + `RUN --mount=type=secret` in the `Dockerfile`), so none end up in image history or layer metadata. `env_file` alone is too late — it only applies at runtime, and the pages that consume these are prerendered. All three come from `.env`, so `docker compose up --build` needs no extra step. Note that BuildKit excludes secret _contents_ from the cache key: after rotating a key, rebuild with `--no-cache` or the old value may be reused.
- **Anything a prerendered page fetches needs its credential at build time.** The rule of thumb: if a page renders remote data and isn't dynamic, a missing token doesn't produce an error — it bakes an empty section into the static output. The homepage shop silently rendered nothing in Docker for exactly this reason.
